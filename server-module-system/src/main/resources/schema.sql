...
CREATE TABLE IF NOT EXISTS `sys_tenant_package` (
    ...
    `deleted`        BIGINT       NOT NULL DEFAULT 0 COMMENT '删除标记（0存在，时间戳表示删除）',
...
CREATE TABLE IF NOT EXISTS `sys_job` (
    ...
    `data`            LONGTEXT     COMMENT '任务默认参数（JSON）',
    `cron_expression` VARCHAR(200)  NOT NULL COMMENT 'Cron表达式',
    `misfire_policy`  TINYINT       NOT NULL DEFAULT 3 COMMENT '错过触发策略（1立即执行 2忽略错过触发 3放弃错过触发）',
    `concurrent`      TINYINT       NOT NULL DEFAULT 0 COMMENT '是否并发执行（0否 1是）',
    `immediate`       TINYINT       NOT NULL DEFAULT 0 COMMENT '创建或更新后是否立即触发一次（0否 1是）',
    `retry_max_attempts` INT         NOT NULL DEFAULT 3 COMMENT '最大执行次数（包含首次执行）',
    `retry_delay_ms` BIGINT         NOT NULL DEFAULT 1000 COMMENT '初始重试延迟（毫秒）',
    `retry_backoff_multiplier` DECIMAL(10,2) NOT NULL DEFAULT 2.00 COMMENT '重试延迟退避倍数',
